...
class GetTheSyncStatusOfDomain(BaseResponse):
    ...
    def post(self, **params):
        ...
        if not manage_confs:
            data = {"domainName": domain, "hostStatus": [{"hostId": host_id, "syncStatus": []}]}
            return self.response(code=SUCCEED, message="no domain conf", data=data)
...
class QuerySupportedConfs(BaseResponse):
    @BaseResponse.handle(schema=QuerySupportedConfsSchema)
    def post(self, **params):
        """
        ...
        if len(conf_files) == 0:
            return self.response(code=SUCCEED, message="get supported conf succeed",
                                 data=yang_conf_list)
